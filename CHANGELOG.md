...
- Inputs: 2-D `.npy`; `.npz` with `hidden_states` / `activations` / `embeddings` or exactly one matrix (optional 1-D `texts` / `observation_ids`); activation JSONL; activation `.json` (array of objects/vectors, or `{ "activations": ... }`)
- `metadata.source` is the activations filename or `--source`, not an absolute path
- Fail closed on rank-3+ dumps, non-finite values, colliding named npz matrices, non-string ids, blank ids, duplicate ids, recursive/non-finite metadata, `--texts` on JSON, and ingesting a file onto itself
