...
- Fail closed on rank-3+ dumps, non-finite values, colliding named npz matrices, colliding JSON `activations`/`rows`, colliding dump aliases, mapping texts/ids, non-string ids, blank ids, duplicate ids, recursive/non-finite metadata, non-object metadata, non-string metadata keys, `--texts` on JSON, ingesting a file onto itself, truncated npz member reads (`BadZipFile` / `EOFError`), and record-row `hidden_state` plus `hidden_states`
