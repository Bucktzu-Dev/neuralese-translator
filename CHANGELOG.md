...
- Fail closed on rank-3+ dumps, non-finite values, colliding named npz matrices, colliding JSON `activations`/`rows`, non-string ids, blank ids, duplicate ids, recursive/non-finite metadata, `--texts` on JSON, and ingesting a file onto itself
