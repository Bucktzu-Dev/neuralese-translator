...
- Python `translate_stream` / `resolve_code` reject the same non-integer stream codes as the CLI (`True`, `0.9`, `"1"`); NumPy integer scalars remain valid codes; exact rationals (`Fraction`) are checked by numerator/denominator before float conversion so a non-integral value cannot round to a code; non-rational reals are checked for integrality in the original precision so an extended type such as `np.longdouble` cannot round through `float()`; overflowing conversions fail as `ValueError`, not `OverflowError`
- `learn`/`ingest` `-o` must differ from the evidence path, including hard links (`Path.samefile`); `learn --parent` is covered by the same identity check, so `-o` cannot replace the parent pack; a symlink-loop output path is a clean `ValueError`, not a traceback
- An unresolved explicit source (`""`, `legacy`, `not-a-checksum`, or an unknown SHA-256) is rejected before current-code lookup, so live code `0` cannot return its current gloss; standalone `rewrite_stream` still selects nothing and returns the original codes
