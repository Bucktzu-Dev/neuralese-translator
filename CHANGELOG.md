...
- Fail closed on rank-3+ dumps, non-finite values, colliding named npz matrices, colliding JSON `activations`/`rows`, colliding dump aliases, mapping texts/ids, non-string ids, blank ids, duplicate ids, recursive/non-finite metadata, non-object metadata, non-string metadata keys, `--texts` on JSON, ingesting a file onto itself, and truncated npz member reads (`BadZipFile` / `EOFError`)
