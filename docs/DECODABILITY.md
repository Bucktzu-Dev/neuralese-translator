...
3. Every non-quarantined symbol has a bound English string. An explicit `[unglossed]` marker may be recorded in the pack (not invented at read time), but it does not satisfy `gloss_bound` unless `--allow-unglossed`.
