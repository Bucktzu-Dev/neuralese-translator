...
v0.1.2 adds `neuralese ingest` for dumped hidden-state matrices. Pack schema and `decoder_version` stay `0.1.1`.
...
- **No in-tree HuggingFace client.** `neuralese ingest` loads `.npy` / `.npz` / activation JSONL you already dumped. It does not download models, run forward passes, or import `transformers`. Rank-3+ token/layer tensors fail closed until you pool them to `(n_observations, hidden_dim)`.
...
If a claim is not certified by `neuralese certify`, it is not part of the audit contract. If `translate` emits English under `default` or `strict`, the pack passed that policy. `--allow-unglossed` still requires evidence and admission. `--allow-uncertified` is an explicit debug override and is not an audit.
